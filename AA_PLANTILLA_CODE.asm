...
;********** RESOLUCIÓN ****************
    ...
    ORG 0X008
    GOTO INT_ALTA_PRIOR; Añadimos el alias de la rutina de servicio para la interrupcion de alta prioridad
    ; Esto sucede en la posición 0x008 del PC
    ORG 0X018
    GOTO INT_BAJA_PRIOR; Hacemos lo mismo para el de baja prioridad
    ORG 0X020
MAIN: ; Damos un alias de MAIN a la posicion 0X020 para el PC 
    ;dentro de esta parte podemos configurar nuestros puertos de trabajo demás cosas
START:; Damos otro alías a otra posición de memoria de programa para iniciar
    ; la rutina que deseamos 
    ORG 0X0C0 
INT_ALTA_PRIOR: ; Damos el alias de INT_ALTA_PRIOR 
    ; a la posicion 0X0C0 para que acá vaya una rutina de servicio a la interrupción
    ; todo ello se verá más adelante
