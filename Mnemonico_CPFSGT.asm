...
;***************** MNEMONICO CPFSGT con el PIC18F4550  ****************************
...
;Esto se entiende por el significado del mnemonico: CPFSGT -> ComPare File Skit if it's Greater Than
;La idea de esta aplicación de este mnemonico es que sí un dato es mayor se moverá un dato a W
;y de no ser mayor, se moverá otro dato.
;********** RESOLUCIÓN ****************
    CBLOCK 0x00
	variablePrueba
    ENDC 
...
START:
    MOVLW .35 ;Movemos un dato literal decimal al registro W
    MOVWF variablePrueba; Luego movemos ese dato hacia variablePrueba
    MOVLW .30; Movemos un dato literal decimal al registro W
    CPFSGT variablePrueba; Luego comprobamos si el dato de variablePrueba es mayor que W
    GOTO MENOR; Si no es mayor vamos a la dirección de memoria de programa con el alias MENOR
    GOTO MAYOR; Si es mayor vamos a la dirección de memoria de programa con el alias MAYOR
